...
resource "aws_ecr_repository" "factory" {
  name                 = "ai-software-factory-simulator"
...
resource "aws_ecr_lifecycle_policy" "factory" {
  repository = aws_ecr_repository.factory.name
  policy = jsonencode({
    rules = [{
      rulePriority = 1
      description  = "Keep the newest 30 release images"
      selection = {
        tagStatus   = "any"
